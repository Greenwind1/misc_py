...
class Model(BaseModel):
    ...
    def build_graph(self, hparams):
        ...
        self.features = tf.placeholder(shape=(None, hparams.feature_nums),
                                       dtype=tf.int32)
        ...
        # DNN
        dnn_input = tf.reshape(emb_inp_v2,
                               [-1, hparams.feature_nums * hparams.k])
        ...
        for idx in range(len(hparams.hidden_size)):
            ...
            W = tf.Variable(np.random.normal(loc=0, scale=glorot, size=(
            input_size, hparams.hidden_size[idx])), dtype=np.float32)
        ...
        W = tf.Variable(np.random.normal(loc=0, scale=glorot,
                                         size=(hparams.hidden_size[-1], 1)),
                        dtype=np.float32)
        dnn_logits = tf.tensordot(dnn_input, W, [[-1], [0]])[:, 0]

        # exFM
        exfm_logit = self._build_extreme_FM(hparams, emb_inp_v2, res=False,
                                            direct=False, bias=False,
                                            reduce_D=False, f_dim=2)[:, 0]
        ...
        self.loss = -tf.reduce_mean(
            self.label * logit_1 + (1 - self.label) * logit_0)
        self.cost = -(self.label * logit_1 + (1 - self.label) * logit_0)
        self.saver = tf.train.Saver()

    def _build_extreme_FM(self, hparams, nn_input, res=False, direct=False,
        ...
        with tf.variable_scope("exfm_part",
            ...
            for idx, layer_size in enumerate(hparams.cross_layer_sizes):
                split_tensor = tf.split(hidden_nn_layers[-1], hparams.k * [1],
                                        2)
                dot_result_m = tf.matmul(split_tensor0, split_tensor,
                                         transpose_b=True)
                dot_result_o = tf.reshape(dot_result_m, shape=[hparams.k, -1,
                                                               field_nums[0] *
                                                               field_nums[-1]])
                ...
                if reduce_D:
                    ...
                    filters_ = tf.get_variable("f__" + str(idx),
                                               shape=[1, layer_size, f_dim,
                                                      field_nums[-1]],
                                               dtype=tf.float32)
                    filters_m = tf.matmul(filters0, filters_)
                    filters_o = tf.reshape(filters_m, shape=[1, layer_size,
                                                             field_nums[0] *
                                                             field_nums[-1]])
                ...
                if direct:
                    ...
                    if idx != len(hparams.cross_layer_sizes) - 1:
                        next_hidden, direct_connect = tf.split(curr_out, 2 * [
                            int(layer_size / 2)], 1)
            ...
            if res:
                ...
                exFM_in = tf.concat([exFM_out1, result], axis=1,
                                    name="user_emb")
