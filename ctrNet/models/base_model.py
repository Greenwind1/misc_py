...
class BaseModel(object):
    ...
    def _get_initializer(self, hparams):
        if hparams.init_method == 'tnormal':
            ...
            return tf.random_uniform_initializer(-hparams.init_value,
                                                 hparams.init_value)
    ...
    def eval(self, T, dev_data, hparams, sess):
        ...
        if hparams.metric == 'logloss':
            ...
            utils.print_out(
                "# Epcho-time %.2fs Eval logloss %.6f. Best logloss %.6f." \
                % (T, log_loss, self.best_score))
        elif hparams.metric == 'auc':
            fpr, tpr, thresholds = metrics.roc_curve(dev_data[1] + 1, preds,
                                                     pos_label=2)
    ...
    def batch_norm_layer(self, x, train_phase, scope_bn):
        bn_train = batch_norm(x, decay=self.hparams.batch_norm_decay,
                              center=True, scale=True, updates_collections=None,
                              is_training=True, reuse=None, trainable=True,
                              scope=scope_bn)
        bn_inference = batch_norm(x, decay=self.hparams.batch_norm_decay,
